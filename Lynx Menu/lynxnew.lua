...
Citizen.CreateThread(function() while bL do Citizen.Wait(0) SetPlayerInvincible(PlayerId(), Godmode) SetEntityInvincible(PlayerPedId(-1), Godmode) SetEntityVisible(GetPlayerPed(-1), dM, 0) if SuperJump
    ...
    if e.IsMenuOpened(bM) then cr("~p~xAries  Revolution - 9.0 - "..d, 0.80, 0.9) a5("Welcome back ~p~"..d..
        " ~s~!", 0.5, 0.5) aM("~h~xAries  Revolution - ~g~9~s~.~g~0", true) aM("~u~https://~s~www.xAries menu.com~u~/", false) if e.MenuButton("~h~~p~#~s~ Self Menu", bO) then elseif e.MenuButton("~h~~p~#~s~ Online Players", bW) then elseif e.MenuButton("~h~~p~#~s~ Teleport Menu", bS) then elseif e.MenuButton("~h~~p~#~s~ Vehicle Menu", bV) then elseif e.MenuButton("~h~~p~#~s~ Weapon Menu", bT) then elseif e.MenuButton("~h~~p~#~s~ Lua Menu ~o~~h~:3", bQ) then elseif e.MenuButton("~h~~p~#~s~ Advanced Mode ~o~~h~xD", bU) then elseif e.MenuButton("~h~~p~# ~y~34ByTe Community", b_) then end; e.Display() elseif e.IsMenuOpened(bO) then a5("Stuff for yourself", 0.58, 0.58) if e.CheckBox("~h~~g~Godmode", Godmode, function(eC) Godmode = eC end) then elseif e.Button("~h~~y~Semi ~g~Godmode") then local eD = "stt_prop_stunt_soccer_ball"
